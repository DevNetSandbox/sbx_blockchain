...
chaincodeQuery () {
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["query","user1"]}' >&log.txt
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -c '{"Args":["invoke","user1","user2","10"]}' >&log.txt
	else
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["invoke","user1","user2","10"]}' >&log.txt
	fi
	res=$?
	cat log.txt
